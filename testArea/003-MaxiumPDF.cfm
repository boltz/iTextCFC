<!--- When creating a PDF Document the page size and margins are defined.
	...
	Example similar to the book iText in Action second edition page 10
	---> 
	
<a href="./bacon-Maximum.pdf">View The Largest Possible PDF</a>
<br>
...
<!--- Set the path --->
<cfset fullPath = expandPath("./bacon-Maximum.pdf")>
...
<cfset pageWidth = javacast("float",14400)>
<cfset pageHeight = javacast("float",14400)>
...
<cfset unit = javaCast("float", 75000)>
...
<!--- set the user unit to correspond to 75,000 points--->
<cfset writer.setUserUnit(unit)>
